...
export class R2StorageService {
  ...
  private initializeS3Client() {
    ...
    const isAwsS3 = this.config.region !== 'auto' && !this.config.region.includes('cloudflare');
  ...
  async uploadFile(
    ...
    try {
      ...
      const result = await this.s3Client.send(command) as PutObjectCommandOutput;
  ...
  async deleteMultipleFiles(keys: string[]): Promise<DeleteResult[]> {
    try {
      ...
      const result = await this.s3Client.send(command) as DeleteObjectsCommandOutput;

      return keys.map(key => ({
        key,
        deleted: !result.Errors?.some((error) => error.Key === key),
        message: result.Errors?.find((error) => error.Key === key)?.Message,
      }));
    } catch (error) {
      throw new TRPCError({
        code: 'INTERNAL_SERVER_ERROR',
  ...
  async listFiles(
    ...
    try {
      ...
      const result = await this.s3Client.send(command) as ListObjectsV2CommandOutput;
   ...
   */
  async headObject(key: string): Promise<{ contentLength: number; contentType?: string; lastModified?: Date }> {
    try {
      ...
      const result = await this.s3Client.send(command) as HeadObjectCommandOutput;
  ...
  private async getImageDimensions(
    ...
    try {
      const inputBuffer = this.convertToBuffer(buffer);
      const metadata = await sharp(inputBuffer).metadata();
      ...
      throw new TRPCError({
        code: 'BAD_REQUEST',
        message: 'Invalid image file or corrupted image data',
      });
    }
  }
...
const r2Service = createR2StorageService({
...
export const fileRouter = createTRPCRouter({
  uploadImage: protectedProcedure
    .input(z.object({
      ...
      category: z.enum(['recipes', 'profiles', 'ingredients']),
    }))
    .mutation(async ({ input, ctx }) => {
      const file = base64ToFileUpload(input.base64, input.filename, input.contentType);
      
      return await r2Service.uploadFile(file, {
  ...
  deleteFile: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      return await r2Service.deleteFile(input.key);
    }),

  listUserFiles: protectedProcedure
    .input(z.object({
      category: z.enum(['recipes', 'profiles', 'ingredients']).optional(),
      maxKeys: z.number().max(100).default(20),
      continuationToken: z.string().optional(),
    }))
    .query(async ({ input, ctx }) => {
      const prefix = input.category ? `${input.category}/${ctx.auth.user.id}/` : `${ctx.auth.user.id}/`;
      return await r2Service.listFiles(prefix, input.maxKeys, input.continuationToken);
    }),
});
*/
